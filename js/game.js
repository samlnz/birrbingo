...
generateBingoCardNumbers(cardNumber) {
    ...
    // Create consistent card for each card number
    ...
    for (let col = 0; col < 5; col++) {
        ...
        while (colNumbers.length < 5) {
            const randomSeed = (seed + col * 100 + colNumbers.length * 10) % 233280;
            const rand = (randomSeed * 9301 + 49297) % 233280 / 233280;
...
const columnColors = ['#FF6B6B', '#4ECDC4', '#45B7D1', '#96CEB4', '#FFEAA7'];
    ...
    <div class="grid-cell ${isFreeSpace ? 'free marked' : ''}" 
         ...
         data-row="${row}" 
         data-col="${col}"
         style="background-color: ${isFreeSpace ? '#FF9E00' : columnColors[col] + '40'}">
...
`;
    ...
    // Immediately claim bingo and go to winner page
    setTimeout(() => {
        this.claimBingo();
    }, 500);
}
        ...
        this.updateCardStats(cardId);
    // Check four corners winning pattern
let cornersComplete = true;
let cornersAllCalled = true;
const corners = [
    [0, 0], [0, 4], [4, 0], [4, 4]  // Four corner positions
];
...
if (cornersComplete && cornersAllCalled && !this.gameState.winningLines[cardId].includes('corners')) {
    ...
    this.highlightWinningLine(cardId, 'corners', 0);
}
}
...
createCalledNumbersGrid() {
    ...
    for (let col = 0; col < 5; col++) {
        ...
        columnDiv.className = 'called-numbers-column';
        columnDiv.style.backgroundColor = columnColors[col] + '20'; // 20 = 12% opacity
        
        // Column header
        const header = document.createElement('div');
        header.className = 'column-header';
        header.textContent = BINGO[col];
        header.style.backgroundColor = columnColors[col];
        columnDiv.appendChild(header);
        
        // Numbers in this column
        for (let num = ranges[col].min; num <= ranges[col].max; num++) {
            const numDiv = document.createElement('div');
            numDiv.className = 'called-number';
            numDiv.textContent = num;
            numDiv.dataset.number = num;
            numDiv.style.backgroundColor = columnColors[col] + '40'; // 40 = 25% opacity
